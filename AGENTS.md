...
**Question-answer bypass:** While `run_turn` blocks on `ask_question`'s oneshot channel, the main `cmd_rx` loop never receives `TuiCmd::Submit` or `QuestionAnswer`. Answers must flow through the `question_answer_tx` side channel (set up in `repl.rs`). The Elm composer routes Enter keypresses through this channel when `active_question` is set. **One question at a time** (invariant): the tool pipeline serializes interactive tools (`ToolRegistry::is_interactive`, currently `ask_question` and `wait_for_user`) as barriers so at most one `QuestionRequested` is ever live — every UI surface tracks a single active question, and two would orphan the first oneshot and freeze the turn (`wait_for_user` never opens a question; it is barrier-serialized as a non-blocking turn-end signal). `AskQuestionTool` also fails fast if a question is already pending, and child subagent sessions have all parent-only tools unregistered via `strip_child_only_tools` (`ask_question`, `wait_for_user`, `spawn_subagent`, the five `task_*` tools; no UI can answer a child question and a grandchild spawn would hang on an undrained oneshot).
