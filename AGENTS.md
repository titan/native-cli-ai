...
**Question-answer bypass:** While `run_turn` blocks on `ask_question`'s oneshot channel, the main `cmd_rx` loop never receives `TuiCmd::Submit` or `QuestionAnswer`. Answers must flow through the `question_answer_tx` side channel (set up in `repl.rs`). The Elm composer routes Enter keypresses through this channel when `active_question` is set. **One question at a time** (invariant): the tool pipeline serializes interactive tools (`ToolRegistry::is_interactive`, currently `ask_question`) as barriers so at most one `QuestionRequested` is ever live — every UI surface tracks a single active question, and two would orphan the first oneshot and freeze the turn. `AskQuestionTool` also fails fast if a question is already pending, and child subagent sessions have `ask_question` unregistered entirely (no UI can answer them; a child question would hang the parent turn).
