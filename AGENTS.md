...
- **Sessions:** `<workspace>/.nca/sessions/<id>.json` (snapshot cache) + `<id>.events.jsonl` (authoritative event log). Resume folds `MessageRecorded` events via `core::replay` (P2 Phase B replay-authoritative; old-format logs fall back to json). `EventLogWriter` fsyncs at each `TurnCompleted` and `run_turn` awaits that commit before returning. IPC socket at `$XDG_RUNTIME_DIR/nca/` (fallback `/tmp/nca/`).
